...
ADR-012 introduced a memory freshness check in the pre-commit gate, ensuring that memory files (`.dev-team/learnings.md` and `.dev-team/agent-memory/*/MEMORY.md`) are actively maintained alongside code changes. ADR-012 itself does not mandate append-only semantics — it specifies an advisory reminder, not a write policy. However, an append-only practice emerged organically: agents produce findings, Borges extracts structured entries, and memory grows over time. No formal write semantics were ever defined.
...
3. **Staleness management**: Agents are instructed to archive or supersede stale entries in their own MEMORY.md at session start. This is a structural mutation.
...
This creates a tension: the memory system's emergent practice is append-only (new learnings accumulate), but the maintenance protocol requires update, move, merge, and remove operations.

Without clarification, agents face contradictory guidance — "always append" vs "clean stale entries" — which leads to either memory bloat (never archiving anything) or silent data loss (removing without audit trail).
...
The only true removal is Borges compressing archived entries into summaries when an agent's MEMORY.md approaches the 200-line cap — and even then, the summary preserves the substance. No agent other than Borges may delete entries outright.

## Consequences

- ADR-012's freshness check remains valid — it ensures memory is updated, not that it is append-only
- The emergent append-only practice is now formalized as the workflow write policy, distinct from maintenance operations
...
- Individual agents retain the ability to archive/supersede their own stale entries at session start, keeping the self-maintenance loop fast
