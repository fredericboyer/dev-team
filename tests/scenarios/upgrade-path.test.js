...
describe("upgrade path scenario", () => {
  it("re-running init preserves customizations and adds new content", async () => {
    ...
    assert.ok(
      updatedClaudeMd.includes("My Custom Rules"),
      "should preserve custom content above markers",
    );
    ...
    assert.equal(
      updatedSettings.customField,
      "user-added",
      "should preserve custom settings fields",
    );
    ...
    for (const [event, entries] of Object.entries(updatedSettings.hooks)) {
      ...
      assert.equal(
        commands.length,
        unique.length,
        `${event} hooks should not have duplicates after re-run`,
      );
    }
  });
  ...
  it("handles project that already has .claude/ with non-dev-team content", async () => {
    ...
    const preToolUseCommands = settings.hooks.PreToolUse.flatMap((e) =>
      (e.hooks || []).map((h) => h.command),
    );
    assert.ok(preToolUseCommands.includes("echo custom"), "should preserve custom hooks");
    assert.ok(
      preToolUseCommands.some((c) => c.includes("dev-team")),
      "should add dev-team hooks",
    );
  });
});
