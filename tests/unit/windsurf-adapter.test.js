...
const { WindsurfAdapter } = require("../../dist/adapters/windsurf");
const { getAdapter } = require("../../dist/formats/adapters");

function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "windsurf-adapter-test-"));
}
...
describe("WindsurfAdapter", () => {
  ...
  it("has correct id and name", () => {
    ...
    assert.equal(adapter.name, "Windsurf");
  });

  it("is registered in the adapter registry", () => {
    const adapter = getAdapter("windsurf");
    assert.ok(adapter, "windsurf adapter should be registered");
    assert.equal(adapter.id, "windsurf");
  });

  it("generate() creates .windsurf/rules/{name}.md files", () => {
    const adapter = new WindsurfAdapter();
    adapter.generate(SAMPLE_DEFS, tmpDir);

    for (const def of SAMPLE_DEFS) {
      const filePath = path.join(tmpDir, ".windsurf", "rules", `${def.name}.md`);
      assert.ok(fs.existsSync(filePath), `${def.name}.md should exist`);
    }
  });

  it("generate() uses YAML frontmatter with description", () => {
    const adapter = new WindsurfAdapter();
    adapter.generate(SAMPLE_DEFS, tmpDir);

    const filePath = path.join(tmpDir, ".windsurf", "rules", "dev-team-voss.md");
