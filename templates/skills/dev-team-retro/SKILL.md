...
- Agent memory that contradicts another agent's memory (e.g., Szabo says "auth uses sessions" but Hopper says "auth uses JWT")
...
- Examples of promotable entries: "all API endpoints require rate limiting" (Szabo), "we always use transactions for multi-table writes" (Hopper), "components must support keyboard navigation" (Hopper)
