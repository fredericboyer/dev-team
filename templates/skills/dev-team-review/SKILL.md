...
| `docker`, `Dockerfile`, `docker-compose`, `.env`, `deploy`, `terraform`, `helm`, `k8s`, `kubernetes`, health check, monitoring | @dev-team-hamilton | Infrastructure |
| `config`, `migration`, `database`, `.sql`, data model | @dev-team-voss | Backend/data layer |
