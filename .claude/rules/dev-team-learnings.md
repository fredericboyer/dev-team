...
- **Agent teams sharing a working directory cause cross-branch contamination.** Recurred in v1.7.0 (stray commits on wrong branches: #447→#438, #436→#446, #440→#434), v1.10.0 (stashes went stale), and v3.3.0 (oxfmt config branch contaminated, redone in worktree). Worktree isolation (one worktree per agent) prevents all of these. #683 tracks worktree hook hardening. Prefer worktree-isolated agents for multi-branch parallel work.
...
- **LIGHT/FULL review tiers validated in practice.** v3.3.0 was the first release to differentiate: SIMPLE tasks got LIGHT (advisory-only) reviews, COMPLEX tasks got FULL reviews. The FULL review on #671 (Mergify ADR) caught the only DEFECT. LIGHT reviews on 6 other branches produced useful advisory findings with zero false-alarm DEFECTs. The tier system correctly concentrates review effort.
- **Never merge with unresolved review threads.** The merge skill (rewritten in v3.2.0, ADR-040) is reviewer-agnostic: query all unresolved threads via GraphQL `reviewThreads`, address each (fix or defer), resolve via `resolveReviewThread` mutation. Mergify enforces `#review-threads-unresolved = 0` at the platform level. Two-phase Copilot polling: wait for workflow to appear (2min), wait to complete (3min). Don't block on human reviewers — set auto-merge and let platform gates enforce.
