...
- **Agent teams sharing a working directory cause cross-branch contamination.** Recurred in v1.7.0 (stray commits on wrong branches: #447→#438, #436→#446, #440→#434), v1.10.0 (stashes went stale), v3.3.0 (oxfmt config branch contaminated, redone in worktree), v3.4.0 (2 PRs picked up files from other branches), v3.5.0 (package.json leaked between branches, fixed manually), and v3.6.0 (stashed changes, agent teams sharing working directory). Worktree isolation prevents cross-branch contamination but not stale-base contamination. Prefer worktree-isolated agents for multi-branch parallel work.
